...
def tunnel_data(conn: socket.socket, overseer_address: str, buffer_size: int, timeout: float):
    with conn:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as connecting_socket:
            with context.wrap_socket(connecting_socket) as ssl_socket:
                ...
                while True:
                    ...
                    if len(client_data_buffer) == 0 and len(server_data_buffer) == 0:
                        break

                    if not client_data and len(client_data_buffer) > 0:
                        ssl_socket.sendall(client_data_buffer)
                        client_data_buffer = bytearray()
                    elif len(client_data_buffer) + len(client_data) > buffer_size:
                        ssl_socket.sendall(client_data_buffer)
                        client_data_buffer = bytearray()
                    ...
                    if not server_data and len(server_data_buffer) > 0:
                        ...
                        server_data_buffer = bytearray()
                    elif len(server_data_buffer) + len(server_data) > buffer_size:
                        conn.sendall(server_data_buffer)
                        server_data_buffer = bytearray()
